...
# test 'run task', {timeout: 1000}, (t) ->
#   t.plan 4

#   server.on 'error', (error) -> t.error error, 'unexpected server error'

#   client = new Client 'ws://localhost:4242'

#   client.on 'error', (error) -> t.error error, 'unexpected client error'

#   client.on 'connect', -> t.pass 'client connected to server'

#   queue = client.queue 'test-queue'

#   task = queue.add {foo: 'bar'}, (error) ->
#     t.error error, 'adding should not error'

#   queue.process (task, callback) ->
#     t.deepEqual task.data, {foo: 'bar'}
#     callback()

#   task.on 'completed', -> t.pass 'task complete'

# test 'task retries', {timeout: 1000}, (t) ->
#   t.end()
